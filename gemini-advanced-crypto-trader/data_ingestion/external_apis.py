# data_ingestion/external_apis.py
import requests
import logging
...
class ExternalAPIs:
    ...
    def get_on_chain_data(self, asset):
        """Fetches key on-chain metrics from Glassnode."""
        ...
        for name, endpoint in metrics.items():
            try:
                ...
                if name == 'mvrv_z_score':
                    ...
                    on_chain_features[name] = 1.0
                else:
                    on_chain_features[name] = 0.5

        logging.info(f"🔗 Fetched on-chain data: {on_chain_features}")
        return on_chain_features

    def get_news_headlines(self, query):
        """Fetches recent news headlines for a given query from NewsAPI."""
