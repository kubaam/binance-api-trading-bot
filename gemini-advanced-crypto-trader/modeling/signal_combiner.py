# modeling/signal_combiner.py
import logging
from config import SIGNAL_WEIGHTS

class SignalCombiner:
    """Combines various signals into a final trading decision (BUY, SELL, HOLD)."""
    ...
    def _get_on_chain_signal(self):
        """Combines MVRV-Z and SOPR for an on-chain signal."""
        score = 0
    ...
    def _get_sentiment_signal(self):
        """Uses the calculated sentiment score directly."""
        return self.data['sentiment_score']

    def _get_forecast_signal(self):
        """Uses the ARIMA forecast to generate a signal."""
    ...
    def generate_final_signal(self):
        ...
        for signal_name, weight in self.weights.items():
            final_score += signals[signal_name] * weight
